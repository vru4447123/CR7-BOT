...
// ══════════════════════════════════════════════════════════════════════════════
//  PERMISSION GUARD
//  Allows: users with Administrator perm OR a role named "Admin Perm" (case-insensitive)
// ══════════════════════════════════════════════════════════════════════════════
function isAdmin(member) {
  if (!member) return false;
  if (member.permissions.has(PermissionFlagsBits.Administrator)) return true;
  return member.roles.cache.some(
    (r) => r.name.toLowerCase() === "admin perm"
  );
}
  ...
  if (!isAdmin(interaction.member)) {
    await interaction.reply({
      embeds: [new EmbedBuilder()
        ...
        .setDescription("You need the **Admin Perm** role or **Administrator** permission to use this command.")],
      ephemeral: true,
    });
    return false;
  }
...
const commands = [
  ...
  new SlashCommandBuilder()
    ...
    .addIntegerOption((o) => o.setName("robux").setDescription("Robux amount in stock").setRequired(true).setMinValue(0)),

  new SlashCommandBuilder()
    .setName("set-stock")
    .setDescription("[Admin] Quickly update the Robux stock number (updates existing embed)")
    .addChannelOption((o) => o.setName("channel").setDescription("Channel the stock embed is in").setRequired(true))
    .addIntegerOption((o) => o.setName("robux").setDescription("New Robux amount").setRequired(true).setMinValue(0)),
  ...
  new SlashCommandBuilder()
    ...
    .addIntegerOption((o) => o.setName("stock").setDescription("Stock quantity (-1 = unlimited)").setRequired(false)),
  ...
  new SlashCommandBuilder()
    ...
    .addStringOption((o) => o.setName("reason").setDescription("Reason").setRequired(false))
    .addIntegerOption((o) =>
      o.setName("delete_days").setDescription("Delete message history (days, 0-7)").setRequired(false).setMinValue(0).setMaxValue(7)
    ),
  ...
  new SlashCommandBuilder()
    ...
    .addStringOption((o) =>
      o.setName("color").setDescription("Embed color (hex, e.g. ff0000)").setRequired(false)
    ),
...
async function cmdLeaderboard(i) {
  ...
  const desc = top.map((u, idx) => {
    const m = idx === 0 ? "🥇" : idx === 1 ? "🥈" : idx === 2 ? "🥉" : `**${idx + 1}.**`;
...
async function cmdCoinflip(i) {
  ...
  won ? db.addCoins(i.user.id, i.user.username, bet) : db.removeCoins(i.user.id, bet);
  return i.reply({
...
async function cmdSlots(i) {
  ...
  let win = 0;
  if (mult > 0) { win = Math.floor(bet * mult); db.addCoins(i.user.id, i.user.username, win - bet); }
  else db.removeCoins(i.user.id, bet);

  return i.reply({
    embeds: [new EmbedBuilder().setColor(mult > 0 ? 0xffd700 : 0xff4444)
      .setTitle("🎰 Slot Machine")
      .setDescription(
        `**[ ${reels.join(" | ")} ]**\n\n${resultText}\n\n` +
        (mult > 0 ? `✅ Won **${formatCoins(win)}** (+${formatCoins(win - bet)})` : `❌ Lost **${formatCoins(bet)}**`) +
        `\n💰 Balance: **${formatCoins(db.getUser(i.user.id).balance)}**`
      )],
  });
}
...
async function resolveDealer(i, userId, state) {
  ...
  if (dv > 21 || pv > dv) { payout = state.bet*2; result = `🏆 **You Win!** Dealer: ${dv}. Won **${formatCoins(payout)}**!`; color = 0x00ff88; }
  else if (pv === dv)      { payout = state.bet;   result = `🤝 **Push!** Bet returned.`;                                     color = 0xffaa00; }
  else                     {                         result = `😞 **Dealer wins** (${dv}). Lost **${formatCoins(state.bet)}**.`; color = 0xff4444; }
...
async function cmdShop(i) {
  ...
  ROBUX_PACKAGES.forEach((p) => {
    embed.addFields({ name: `🎮 ${p.name}`, value: `🪙 **${p.coins.toLocaleString()} LEN** → **${p.robux} R$**\n\`/buy ${p.name}\``, inline: true });
  });
  ...
  if (extras.length) {
    ...
    extras.forEach((it) => {
      ...
      embed.addFields({ name: `${it.emoji || "📦"} ${it.name} — 🪙 ${it.price.toLocaleString()} LEN`, value: `${it.description}\nStock: ${stock}\n\`/buy ${it.name}\``, inline: true });
    });
  }
...
async function cmdBuy(i) {
  ...
  if (pkg) {
    ...
    if (user.balance < pkg.coins)
      return i.reply({ content: `❌ Need **🪙 ${pkg.coins.toLocaleString()} LEN** — you have **${formatCoins(user.balance)}**.`, ephemeral: true });
  ...
  if (user.balance < item.price)
    return i.reply({ content: `❌ Need **${formatCoins(item.price)}** — you have **${formatCoins(user.balance)}**.`, ephemeral: true });
  db.removeCoins(i.user.id, item.price);
  db.addToInventory(i.user.id, item.name);
  if (item.stock > 0) db.decrementStock(item.name);
  return i.reply({
    embeds: [new EmbedBuilder().setColor(0x00ff88).setTitle("✅ Purchase Successful!")
      .setDescription(`Bought **${item.emoji || "📦"} ${item.name}** for **${formatCoins(item.price)}**\n💰 Balance: **${formatCoins(db.getUser(i.user.id).balance)}**`)],
  });
}
...
async function cmdInventory(i) {
  ...
  if (!inv.length)
    return i.reply({ embeds: [new EmbedBuilder().setColor(0x7289da).setTitle(`🎒 ${target.username}'s Inventory`).setDescription("Empty! Use `/shop` to browse.")] });
  const grouped = {};
...
async function cmdShowStock(i) {
  ...
  if (existing) {
    try {
      ...
      return i.reply({ content: `✅ Stock updated in <#${channel.id}> → **${robux.toLocaleString()} R$**`, ephemeral: true });
    } catch { /* deleted, post fresh */ }
  }
  const msg = await channel.send({ embeds: [embed] });
  db.setStockMessage(channel.id, msg.id);
  return i.reply({ content: `✅ Stock embed posted in <#${channel.id}> → **${robux.toLocaleString()} R$**`, ephemeral: true });
}
...
async function cmdSetStock(i) {
  ...
  const existing = db.getStockMessage(channel.id);
  if (!existing)
    return i.reply({ content: `❌ No stock embed found in <#${channel.id}>. Use \`/show-stock\` first.`, ephemeral: true });
  try {
    const msg = await channel.messages.fetch(existing);
    await msg.edit({ embeds: [buildStockEmbed(robux, i.user)] });
    return i.reply({ content: `✅ Stock updated → **${robux.toLocaleString()} R$** in <#${channel.id}>`, ephemeral: true });
  } catch {
    db.setStockMessage(channel.id, null);
    return i.reply({ content: `❌ Couldn't find the embed. Use \`/show-stock\` to post a new one.`, ephemeral: true });
  }
}
...
async function cmdAddItem(i) {
  ...
  db.addShopItem(item);
  return i.reply({ embeds: [adminEmbed(`✅ Added **${item.emoji} ${item.name}** — **${formatCoins(item.price)}** (Stock: ${item.stock === -1 ? "∞" : item.stock})`)] });
}
...
async function cmdTimeout(i) {
  ...
  if (!target) return i.reply({ content: "❌ User not found in this server.", ephemeral: true });
  if (target.id === i.user.id) return i.reply({ content: "❌ You can't timeout yourself.", ephemeral: true });
  if (!target.moderatable) return i.reply({ content: "❌ I can't timeout this user (check my role hierarchy).", ephemeral: true });
...
async function cmdWarnings(i) {
  ...
  if (!warnings.length)
    return i.reply({ embeds: [new EmbedBuilder().setColor(0x00ff88).setTitle(`⚠️ ${target.username}'s Warnings`).setDescription("No warnings on record.")] });

  const desc = warnings.map((w, idx) =>
...
async function cmdKick(i) {
  ...
  try { await target.user.send({ embeds: [new EmbedBuilder().setColor(0xff4444).setTitle(`👢 You were kicked from ${i.guild.name}`).setDescription(`**Reason:** ${reason}`)] }); } catch {}
...
async function cmdBan(i) {
  ...
  try { await target.user.send({ embeds: [new EmbedBuilder().setColor(0xff0000).setTitle(`🔨 You were banned from ${i.guild.name}`).setDescription(`**Reason:** ${reason}`)] }); } catch {}
...
async function cmdPurge(i) {
  ...
  const toDelete = [...messages.values()].slice(0, amount).filter(
...
async function cmdAnnounce(i) {
  ...
  let color = 0x5865f2;
  if (colorHex) { const parsed = parseInt(colorHex.replace("#",""), 16); if (!isNaN(parsed)) color = parsed; }
  ...
  return i.reply({
    embeds: [new EmbedBuilder()
      ...
      .addFields(
        ...
        { name: "💰 LEN Coins",  value: formatCoins(balance),                                                inline: true  },
  ...
  return i.reply({
    embeds: [new EmbedBuilder()
      ...
      .addFields(
        ...
        { name: "🔒 Verif.",    value: g.verificationLevel.toString(),                          inline: true  },
        { name: "💎 Boost",     value: `Tier ${g.premiumTier} (${g.premiumSubscriptionCount})`, inline: true  },
      )
      .setTimestamp()],
  });
}
...
function adminEmbed(desc) {
  return new EmbedBuilder().setColor(0x7289da).setTitle("🔧 Admin Action").setDescription(desc).setTimestamp();
}
...
function handValue(h) { let t = h.reduce((s,c)=>s+c.value,0), a = h.filter(c=>c.rank==="A").length; while(t>21&&a>0){t-=10;a--;} return t; }
